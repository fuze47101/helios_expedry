...
// load-cell mount boss — stepped standoff on the BACK face (the face OPPOSITE the slot),
// so it mounts up to the load cell with the frame hanging straight down below it.
LC_TIP_W   = 8;     // tip width along the channel (X) — seats in the 8 mm gap between the load-cell screws
LC_BASE_W  = 16;    // shoulder width (X)
LC_TIP_ZH  = 8;     // tip height (Z)
LC_BASE_ZH = 10;    // shoulder height (Z)
LC_BASE_H  = 10;    // shoulder protrusion off the back face (+Y) — clears the 10 mm safety screw
LC_TIP_H   = 12;    // tip protrusion (+Y) total
LC_SCREW_D = 5.5;   // attach-screw clearance — SET to your load-cell thread (M5=5.5, M4=4.5, M6=6.5)
LC_HEAD_D  = 10;    // screw-head recess Ø
LC_HEAD_H  = 2.8;   // screw-head recess depth
...
module c_channel(){
    ...
    ceil   = CH_WALL + slotZ;             // slot ceiling z
    difference(){
        union(){
            cube([CH_LEN, outerY, outerZ]);
            // stepped mount boss on the BACK (+Y) face, dead center, centered in height:
            // 16x10 shoulder (protrudes 0-10mm in +Y, clears the 10mm safety screw) ->
            // 8x8 tip (10-12mm, seats in the 8mm gap between the load-cell screws).
            translate([CH_LEN/2, outerY-EPS, outerZ/2]){
                translate([-LC_BASE_W/2, 0, -LC_BASE_ZH/2]) cube([LC_BASE_W, LC_BASE_H+EPS, LC_BASE_ZH]);
                translate([-LC_TIP_W/2, LC_BASE_H, -LC_TIP_ZH/2]) cube([LC_TIP_W, (LC_TIP_H-LC_BASE_H)+EPS, LC_TIP_ZH]);
            }
        }
        // tab slot: open at X=0 (insert) and Y=0 (front, frame hangs straight down out here); far X end = stop
        translate([-EPS, -EPS, CH_WALL]) cube([CH_LEN-CH_WALL, slotY, slotZ]);
        // LOCK holes (vertical) through top lip + tab + bottom lip, aligned at full seat
        ...
        // ATTACH screw: +Y through the back wall + boss into the load-cell thread; head
        // recessed into the slot's back wall so it never blocks the sliding tab ("subset").
        translate([CH_LEN/2, slotY-EPS, outerZ/2]) rotate([-90,0,0]) cylinder(d=LC_SCREW_D, h=(outerY-slotY)+LC_TIP_H+2*EPS, $fn=32);
        translate([CH_LEN/2, slotY-EPS, outerZ/2]) rotate([-90,0,0]) cylinder(d=LC_HEAD_D, h=LC_HEAD_H+EPS, $fn=32);
    }
}
